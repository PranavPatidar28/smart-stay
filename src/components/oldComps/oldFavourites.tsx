...
export default function FavoritesPage() {
    ...
    const fetchFavorites = async () => {
      ...
      try {
        ...
        setFavorites(data.favorites.map((favorite: any) => ({
          ...
          amenities: favorite.property.amenities?.map((a: any) => a.amenity?.name).filter(Boolean) || [],
          addedDate: favorite.createdAt,
          landlord: {
            name: favorite.property.owner?.name || '',
            rating: favorite.property.owner?.rating || 0,
            verified: favorite.property.owner?.verified || false
          }
        })));
      } catch (err) {
  ...
  const mapFavoriteToPropertyCard = (fav: FavoriteProperty) => ({
    ...fav,
    images: fav.images ? fav.images.map((url, i) => ({ id: i.toString(), url, order: i, isCover: i === 0 })) : fav.image ? [{ id: "0", url: fav.image, order: 0, isCover: true }] : [],
    amenities: (fav.amenities || []).map((a: any) => ({ amenity: { id: a.amenity?.name || 'unknown', name: a.amenity?.name || 'Unknown', category: 'Custom' } })),
    owner: fav.landlord ? { 
      id: fav.id, 
      name: fav.landlord.name, 
      verified: fav.landlord.verified, 
      rating: fav.landlord.rating 
    } : { 
      id: fav.id, 
      name: '', 
      verified: false 
